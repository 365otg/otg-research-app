...
The capital city of Agartha is the legendary Shamballa, from which the "King of the
 ...
 light that endure through the dark ages of the surface world.
...
Enlil was a stern, pragmatic, and authoritarian leader who prioritized mission
 ...
 authoritarian, law-giving sky-god (Gnostic Demiurge), who seeks to maintain
...
Yakub's goal was to "graft" the white race from the Original Black Nation. By enforcing
 ...
 the allegorical biblical tale of Jacob using peeled rods to influence the breeding of
...
The cornerstone of the Great Deception is cosmological: the replacement of the true,
 ...
 The entire chronicle is an an effort to expose and dismantle this Great Deception.
...
const App = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-neutral-950 text-neutral-200 font-inter md:flex-row">
      {/* Mobile Header and Sidebar Toggle */}
      <header className="md:hidden flex items-center justify-between p-4 bg-neutral-900 border-b border-neutral-700">
        <div className="flex-grow">
          <h1 className="text-xl font-bold text-indigo-400">OTG Research</h1>
          <p className="text-xs text-indigo-300">By Charles Paris</p>
        </div>
        <button
          onClick={() => setIsMenuOpen(!isMenuOpen)}
          className="p-2 text-neutral-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-md"
      ...
      {/* Left Sidebar - Full display on desktop, hidden on mobile by default */}
      <aside className={`md:w-1/4 bg-neutral-900 border-r border-neutral-700 overflow-y-auto p-4 flex-col rounded-r-lg shadow-lg ${isMenuOpen ? 'flex absolute inset-0 z-40' : 'hidden md:flex'}`}>
        {/* Desktop Header */}
        ...
        <div className="text-center md:hidden mb-4">
          <p className="text-sm text-indigo-300">12-20-2020 - Last updated July 31, 2025</p>
          <p className="text-xs text-indigo-300">{currentDateTime}</p>
        </div>
        <div className="text-center hidden md:block">
          <p className="text-sm text-indigo-300 mb-1">12-20-2020 - Last updated July 31, 2025</p>
          <p className="text-xs text-indigo-300 mb-6">{currentDateTime}</p>
        </div>
